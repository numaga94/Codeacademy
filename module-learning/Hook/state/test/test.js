...
describe('', function () {
  ...
  it('', function () {
    ...
    try {
      ...
      expect(false, "Check for errors in your `AppFunction` component, it isn't rendering");
    }
    ...
    try {
      ...
      expect(false, 'Your `AppFunction()` should import and render `<NewTask />` and `<TasksList />`').to.be.true;
    }
    ...
    expect(
      newTask.title === USER_INPUT.title && newTask.description === USER_INPUT.description,
      'Manage the state of the title and description for a new task in your `AppFunction` component'
    ).to.be.true;

    expect(wrapper.find('TasksList').prop('allTasks').length, '`<TasksList />` should start as an empty list').to.equal(
      0
    );
    ...
    function submitNewTodoForm() {
      try {
        ...
        expect(false, 'Submitting the form should not throw an error').to.be.true;
      }
    }
    ...
    expect(allTasks[0].hasOwnProperty('id'), 'Each object in the `allTasks` array should have a unique `id` property')
      .to.be.true;

    newTask = wrapper.find('NewTask').prop('newTask');
    expect(
      (!newTask.title && !newTask.description) || (newTask.title === '' && newTask.description === ''),
